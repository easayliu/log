...
* **异常堆栈会自动合并**到上一条日志的 `message`，不会被拆成一堆碎片。单条有上限
  （`parser.max_message_lines` 500 行、`parser.max_message_bytes` 256KiB），**超了就整条
  封口**：后面的续行一律不再接，末尾补一行 `... [logpipe 截断：省略 N 行 / M 字节]`。
  不封口而「挑装得下的短行继续接」的话，message 里会出现看不出来的断层（第 300 行
  直接接第 480 行），那比少一段更糟；丢了不留痕也一样 —— 库里是一条看着挺完整的日志，
  下游无从知道少了东西；
...
丢掉这种超长行时会**同时给聚合器封口**：丢的是这条巨行的前半截，后半截还在文件里，
不封口的话缓冲清空后遇到的第一个换行切出来的就是那截残尾 —— 它解析不出头部，会被
当成续行挂到一条毫不相干的日志上。封了口之后这段残尾直接丢掉，只在上一条的末尾
记一笔 `... [logpipe 截断：...]`。
